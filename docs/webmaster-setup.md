...
- [ ] 百度/360/神马已交 sitemap；搜狗已逐条交 URL（搜狗无 sitemap）
- [ ] GA4 实时有访问
...
**说明**：
- 360 / 搜狗 / 神马 **没有** 与百度同级的公开「Token 推送 API」给普通个人站。
- **搜狗仅支持页面 URL 提交，不支持 sitemap 文件**；360 / 神马可交 sitemap，收录仍以抓取为主。
...
### 9.5 验证通过后：提交资源（仅 URL，无 sitemap）

> **搜狗限制**：只支持 **页面对应链接** 的提交，**不支持 sitemap 文件** 批量提交。不要去找 sitemap 入口。

1. 菜单通常有 **搜索服务 → 资源提交**（或「链接提交 / URL 提交」）。
2. 逐条粘贴完整 URL（一天有配额，优先发新文）：
   - 首页：`https://blog.deltastudio.space/`
   - 最新中文文章 URL
   - 重要分类/系列页（可选）
3. 提交后看「已提交 / 已收录」状态。
...
```text
部署 → 确认新 URL 200
  → 搜狗后台「资源/链接提交」逐条贴 URL（用当日配额）
  → 等待 Sogou spider / Sosospider（robots 已放行）
```

无通用个人站公开 Token API；只能靠后台 **URL 提交** + 自然抓取。
...
| 找不到 sitemap 入口 | **正常**——搜狗不收 sitemap 文件，只交 URL |
| 收录很少 | 新站常态；保持中文内容更新 + 内链 + 持续 URL 提交 |
...
4. 搜狗：注册 → 添加站点 → meta/文件验证 → **逐条 URL 提交**（无 sitemap）
5. 神马：注册 → 添加站点 → 代码/文件验证 → 提交 sitemap
6. 之后每次发文：publish-seo.ps1（IndexNow + 百度 API）
   → 360/神马 靠 sitemap 重抓；搜狗 后台逐条 URL 提交
```
