...
> 说明：以上按官方示例检查方法是否挂在原型上——若返回 `false`，说明浏览器未实现原生 API，会走 polyfill 兜底路径。
...
## 09 课后作业：悬挂吊灯照亮 HTML 表面 3D 场景

**作业目标**：实现一个"悬挂吊灯照亮 HTML 表面"的交互式 3D 场景——一盏由聚光灯（spot）与点光灯泡（bulb）共同点亮的悬挂吊灯装置，对标 AwardWebsites 中新增的 `html-in-canvas-light` demo：HTML-in-Canvas 渲染 + Three.js 3D + Verlet 钟摆物理 + 完整鼠标交互。
...
左键拖拽吊灯使其摆动、并操作灯下 HTML 控制面板，松手后吊灯按惯性自然甩出去（动图来自 AwardWebsites 中的 `html-in-canvas-light` demo）：

![左键拖拽吊灯与 HTML 表面交互动效](/images/2026-09-02_html-in-canvas/drag-swing-interaction.gif)

### 完整实现流程

整个 demo 用**三个** **`useEffect`** **划分"初始化 → 场景 → 状态同步"三阶段**，外加一个固定步长渲染循环，每一阶段都能在 `index.tsx` 里找到对应代码，是理论落地的直接范本：

**阶段 0｜初始化 polyfill**（`useEffect` ①）

```tsx
canvasRef.current.setAttribute('layoutsubtree', '')   // 把 HTML-in-Canvas 能力绑定到 canvas
await import('three-html-render/polyfill')            // 动态安装 polyfill
installHtmlInCanvasPolyfill()                         // 注册回退渲染路径
installThreeHtmlTextureCompatibility()                // 纹理上传兼容层
```
...
pageTexture = new THREE.HTMLTexture(pageSource)      // 真实 DOM 控制面板 → 纹理
...
// 灯光整备：聚光灯(spot)与点光灯泡(bulb)是同一吊灯装置的两层灯光，
// 控制面板的每个参数最终都同步到这组引用
rig = { spot, bulbLight, bulbMaterial, glowMaterial, undersideMaterial }
interactions.add(pageMesh)     // InteractionManager：DOM 事件 → in-canvas HTML 表面
```

**阶段 2｜约束 Verlet 钟摆**（物理内核，固定步长 `1/120`）

```ts
// 固定步长累加器：渲染帧率波动不扰动物理稳定性
accumulator += Math.min(delta, 0.05)
while (accumulator >= 1/120) { stepPhysics(); accumulator -= 1/120 }

velocity  = (position - previous) * (pulling ? 0.985 : 0.9948)  // 阻尼
position += velocity + gravity * (1/120)²                       // 重力 -9.81
// 拖拽 = 沿指针 3D 平衡方向施加隐形弹簧力，强度 52 * (1/120)²
// 每步把 |position - anchor| 约束回绳长 ropeLength = 1.22
```

**阶段 3｜鼠标交互**

```text
左键拖拽   更新指针目标 updatePointerTarget → 与交互平面求交得到 aimTarget；
           指针/吊灯在 NDC 上的距离 → pullStrength = smoothstep(距离, 0.08, 1.15)
松手       动量 = 当前动量 + 指针动量(transfer 0.055–0.12) + 回弹冲量(0.32–1.6)
           clampLength(4.25)、previous 错位一步 → 吊灯自然甩出去
右键拖拽   左右移动调整光束角，clamp(16°, 58°)
右键单击   未拖动才算单击，在 5 个 COLOR_PRESETS 间循环切换颜色
双击       resetMotion() 复位吊灯与灯光
```
...
canvas.requestPaint?.(); wakeRef.current?.()   // 触发重绘并唤醒渲染
```
...
```ts
animate(time) {
  固定步长累计 → 最多追补 5 步；updateRig(); interactions.update(); render(scene, camera)
  if (pulling || stableFrames < 80 || frame < 4) requestAnimationFrame(animate)
  // 否则休眠：稳定 80 帧后停帧，避免持续占用 GPU
}
wake() { stableFrames = 0; 若未在跑则重启 requestAnimationFrame }
// paint 事件、任何交互、resize 都会调用 wake() 唤醒
```

**阶段 6｜兼容性与 HMR 兜底**

```ts
onNativeUploadFailed(el => {                       // 捕获 “No cached paint record”
  setTimeout(() => { pageTexture.needsUpdate = true
                     canvas.requestPaint?.(); wake() }, 0)  // 延迟一拍，避免 version 被覆盖
})                                                  // 最多重试 60 次
void document.fonts.ready.then(() => {
  canvas.requestPaint?.(); resize(); setReady(true)    // 字体就绪后再出图，避免闪白
})
```
