...
**超采样抗锯齿**：直接算覆盖率很难，于是用**在像素内多次采样并平均**来近似 1 像素盒式滤波的效果

![4x4 超采样示意](/images/2026-07-21_series_games101/04_rasterization/chap4_37.png)

**步骤**：

1. 把每个像素细分为 $N \times N$ 个子采样点
2. 对每个子采样点判断是否在三角形内（1 或 0）
3. 对 $N \times N$ 个结果取平均，作为该像素的值

![超采样 Step 1 取样](/images/2026-07-21_series_games101/04_rasterization/chap4_38_1.png)
...
**缺点**：计算量增大 $N^2$ 倍（天下没有免费的午餐，MSAA 的代价就是更多采样开销）
...
| 技术 | 原理 | 优点 | 缺点 | 适用场景 |
|:---:|:---|:---|:---|:---|
| **MSAA** | 每个像素细分为 $N \times N$ 子采样点，对每个子采样点判断 inside(triangle)，按覆盖率取平均，近似 1 像素盒式预滤波 | 效果好、原理清晰、几何边缘平滑 | 计算量随 $N^2$ 增长、对 shader 与纹理走样无效 | 几何边缘抗锯齿、传统引擎默认 AA |
